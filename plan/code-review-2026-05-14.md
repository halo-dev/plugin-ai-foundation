...
### ~~20. AiModel 的 `supportedTextDelta` 使用 `Boolean` 包装类~~ ✅ FIXED
...
### ~~24. ProviderListItem 没有显示 provider 的启用/禁用状态~~ ✅ FIXED
...
### ~~25. ProviderDetail 连通性检查失败无 UI 反馈~~ ✅ FIXED
...
### ~~26. 模型列表没有显示 enabled 状态和能力标签~~ ✅ FIXED
...
### ~~28. EmbeddingModelImpl 的 `partition` 方法对 `size <= 0` 处理不当~~ ✅ FIXED
