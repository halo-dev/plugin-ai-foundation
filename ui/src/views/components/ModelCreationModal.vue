...
const { mutate, isPending } = useMutation({
  mutationFn: async (formState: ModelFormState) => {
    const generateName =
      `${props.providerName}-${formState.modelId.replace(/\//g, '-')}-`.toLocaleLowerCase()
  ...
  onSuccess: () => {
    ...
    modal.value?.close()
    queryClient.invalidateQueries({ queryKey: [QK_MODELS, props.providerName] })
  },
})
...
<template>
  <VModal
    ...
    @close="emit('close')"
  >
    <ModelForm ref="form" @submit="onSubmit" />
    <template #footer>
      <VSpace>
