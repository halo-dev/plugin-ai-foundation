...
function updateNumberField(key: NumberField, value: string) {
  ...
  switch (key) {
    case 'temperature':
      emit('update:temperature', number as number)
      break
    case 'topP':
    ...
    case 'maxTokens':
      ...
      break
    case 'seed':
      emit('update:seed', number)
      break
    case 'maxRetries':
      emit('update:maxRetries', number)
  }
}
</script>
        ...
        <div class=":uno: grid grid-cols-2 gap-2">
          ...
          <div class=":uno: space-y-1">
            ...
            <input
              ...
              step="1"
              class=":uno: w-full text-slate-700 outline-none !border !border-slate-200 !rounded-md !border-solid !bg-white !px-2 !py-1.5 !text-xs placeholder:text-slate-400 focus:!border-teal-400 placeholder:!text-xs focus:!ring-3 focus:!ring-teal-500/10"
              @input="updateNumberField('maxRetries', ($event.target as HTMLInputElement).value)"
            />
          </div>
        </div>
        <div
        ...
        <template v-if="outputMode === 'OBJECT' || outputMode === 'ARRAY'">
          <div class=":uno: space-y-1">
            ...
            <textarea
              ...
              @input="emit('update:outputSchemaText', ($event.target as HTMLTextAreaElement).value)"
            />
            <div
              class=":uno: text-[10px]"
