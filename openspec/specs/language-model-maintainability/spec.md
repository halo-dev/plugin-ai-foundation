...
The language model implementation SHALL centralize tool-step orchestration so streaming and non-streaming tool loops use the same resolution rules for executable tools, external tools, approval requests, tool repair, warnings, and provider continuation.
...
- **AND** behavior for external tools, approval-required tools, and repaired tool calls SHALL remain consistent across both execution modes
