...
Halo CMS 插件生态系统目前缺乏集中式的 AI 基础设施。现有的 `plugin-ai-assistant` 实现了自己的提供商配置和 AI 客户端管理。任何其他需要 AI 能力的插件都必须重复这项工作，迫使用户反复重新配置 API 密钥。
...
- 在不放弃 Halo Extension 资源建模的前提下，为后续 Console workspace 预留接近工作区式产品的厂商/模型配置体验
...
**决策：** 采用两个 Extension 分别管理提供商配置和模型定义。消费者通过 `${providerResourceName}/${modelId}` 格式定位模型，其中 `providerResourceName` 对应 `AiProvider.metadata.name`，而不是 `providerType`；后续 Console UI 再按 provider 聚合展示其配置和关联模型，形成工作区式的 provider workspace。
...
- 不把 `models` 内嵌到 `AiProvider`，仍可通过 provider 聚合查询构建右侧详情页
...
- 仅用 `Map<String, String> config` 难以支撑工作区式产品那种表单交互、字段校验和敏感信息处理
...
- 后续如需兼容工作区式产品那种多 key 体验，可约定在同一个 Secret value 中使用逗号分隔多个 key，由 provider adapter 自行解析
...
**理由：**
- 工作区式产品的模型管理体验依赖分组、能力标签和端点类型来支持搜索、筛选和编辑
...
### 8. 工作区式的 provider workspace（后续 change）

**决策：** 在后续 `ai-foundation-console` change 中，Console UI 采用左侧 provider 列表、右侧 provider 详情与模型列表的工作区布局，而不是将 provider 和 model 完全拆成两个割裂的管理页面。
...
**理由：**
- 借鉴成熟 SDK 的 `LanguageModelV4` / `EmbeddingModelV4` 分离设计
...
**理由：**
- 借鉴成熟 SDK 的 `LanguageModelV4StreamPart` 标准化设计
...
**理由：**
- 借鉴成熟 SDK 的 `providerOptions` 命名空间设计
...
**理由：**
- 借鉴成熟 SDK 的统一错误基类 + marker pattern
...
- `embed()` 是否应该支持批量大小限制或对大输入列表的自动分块？
  - **初步结论：** 借鉴成熟 SDK 的 `embedMany` 设计，`EmbeddingModel` 接口暴露 `maxEmbeddingsPerCall()` 和 `supportsParallelCalls()` 属性。`AiModelServiceImpl` 内部实现自动分块和并行调用，消费者无需感知批量限制。
