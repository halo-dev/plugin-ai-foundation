...
- Implement structured streaming semantics aligned with provider-neutral AI API:
...
- Do not attempt full third-party UI stream protocol stream protocol compatibility; Halo keeps its own stream protocol while aligning Core semantics.
...
- `structured-output-generation`: Add external provider-neutral structured streaming behavior for partial output and array element streams.
