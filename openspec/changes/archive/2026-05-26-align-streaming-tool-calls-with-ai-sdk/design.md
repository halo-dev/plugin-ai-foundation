...
- No AI SDK compatibility mode and no Vercel stream headers.
...
2. Treat tool calls as step-completion data.
   ...
   Alternative considered: always add AI SDK-style `tool-call-streaming-start` and `tool-call-delta` equivalents. That would be misleading for providers that only expose completed calls through Spring AI.
