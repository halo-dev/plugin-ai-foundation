...
AI Foundation already exposes the main AI SDK Core-inspired surfaces: `generateText`, `streamText`, request-scoped tools, reasoning parts, structured output, warnings, usage, metadata, and steps. The remaining gap is behavioral depth. Some fields exist but do not yet provide the same level of runtime semantics that AI SDK callers expect, especially around stream result composition, structured streaming, tool execution context, warnings, and validation error details.
...
- Implement AI SDK UI stream protocol compatibility.
...
Rationale: AI SDK's `streamText` returns a rich result with `textStream`, `fullStream`, `partialOutputStream`, `elementStream`, complete `output`, and final metadata. A direct `Flux<TextStreamPart>` cannot represent these surfaces without either re-consuming the stream or hiding important state.
...
Rationale: AI SDK streams are multiple views over one generation. Multiple subscriptions must not duplicate provider requests or tool execution.
...
Rationale: This keeps the earlier decision that final structured results belong to result objects, while still implementing AI SDK-like partial and element streaming.
...
Rationale: AI SDK differentiates partial output from complete validated array elements. Halo should avoid claiming a partial object is fully schema-valid while still enabling progressive UI.
...
Rationale: AI SDK forwards tool call id and messages to tool execution. Real tools need correlation and state, especially when streaming custom status or invoking nested model calls.
...
Alternative considered: fail fast for every unsupported option. Rejected because AI SDK often reports provider limitations as warnings when generation can still proceed.
