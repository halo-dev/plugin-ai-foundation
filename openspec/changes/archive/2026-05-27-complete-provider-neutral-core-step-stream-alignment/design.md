...
- Add provider-neutral step control equivalent to provider-neutral AI API's `stopWhen` and `prepareStep`.
...
Alternative considered: leave callers to call `result()` and unpack everything manually. That works, but is noticeably less aligned with provider-neutral AI API's promise-based stream result.
...
Alternative considered: add dedicated `output-*` stream parts. That was rejected earlier because provider-neutral AI API's `streamText` structured output is exposed as result projections, not extra full-stream parts.
