...
- Extend stream protocol coverage for provider-neutral AI API-like events that are currently missing or implicit, especially `source`, `file`, `tool-input-start`, and `tool-input-delta`.
...
- `streaming-tool-calls`: Replace max-step-only looping with external provider-neutral step control and expose tool input streaming events when available.
