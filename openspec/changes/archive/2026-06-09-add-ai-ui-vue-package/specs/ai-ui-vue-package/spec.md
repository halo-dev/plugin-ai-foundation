...
- **AND** the message history sent to the backend SHALL exclude the regenerated assistant output
...
#### Scenario: Add tool result
- **WHEN** a caller adds a tool result for a pending tool call
- **THEN** the package SHALL append or update a `tool-result` part associated with the matching assistant message
...
- **WHEN** a caller responds to a tool approval request
- **THEN** the package SHALL append or update a `tool-approval-response` part for the matching approval id and tool call id
