...
- request messages are plain role/content strings, which will be hard to evolve toward external provider-neutral model messages, tools, and multimodal parts;
...
AI Foundation should make the caller contract stable before other plugins build on it. The new contract should align with provider-neutral AI API's `generateText` / `streamText` concepts while staying independent of vendor-specific headers and independent of Spring AI public types.
...
- Introduce external provider-neutral `ModelMessage` and content-part types. V1 accepts text parts only, while preserving an extensible shape for future image, file, tool, and reasoning support.
...
- This change does not attempt to be wire-compatible with third-party UI stream protocol. It borrows useful stream-part semantics but uses Halo protocol identity.
