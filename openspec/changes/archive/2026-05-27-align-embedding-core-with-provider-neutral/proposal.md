...
The current embedding API is usable, but it is still much thinner than the text generation surface and provider-neutral AI API's embedding contract. It returns vectors but lacks request-scoped diagnostics, retry/header controls, meaningful provider option mapping, response metadata, and utility behavior that callers need for production RAG and similarity workflows.

## What Changes

- Align embedding calls with provider-neutral AI API's `embed` and `embedMany` behavior while preserving Halo's Java API shape:
...
- Add an explicit embedding settings surface aligned with provider-neutral AI API's Embeddings `Settings` section:
...
- `embedding-core-alignment`: external provider-neutral embedding request/response semantics, provider options, retry/header controls, parallel controls, diagnostics, and utility behavior.
