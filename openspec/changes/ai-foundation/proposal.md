...
- **资源保持分离，交互保持聚合**：存储层继续使用 `AiProvider` 与 `AiModel` 两类 Extension；UI 层按 provider 聚合展示其配置与关联模型，以兼顾 Halo Extension 设计和 Cherry Studio 式产品体验
...
- `console-model-management`：用于管理 AI 提供商配置和模型定义的 Vue Console UI，交互方式参考 Cherry Studio 的 provider workspace
