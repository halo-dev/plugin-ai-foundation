...
class ProviderTypeConsoleEndpointTest {
    ...
    void listProviderTypes_describesProviderOwnedAdaptersAndRecommendations() {
        ...
        webTestClient.get().uri("/provider-types")
            ...
            .jsonPath("$[0].adapters[1].defaultParameterMappingOverrides.REASONING.template")
    ...
    void listProviderTypes_includesNativeRerankAdapterMetadata() {
        ...
        webTestClient.get().uri("/provider-types")
            ...
            .value(types -> org.assertj.core.api.Assertions.assertThat(types.toString())
                .contains("dashscope-compatible-rerank", "dashscope-native-rerank"))
            .jsonPath("$[?(@.providerType == 'dashscope')].adapters"
                + "[?(@.adapterType == 'dashscope-compatible-rerank')].displayName")
            .isEqualTo("DashScope 兼容重排")
            .jsonPath("$[?(@.providerType == 'dashscope')].adapters"
                + "[?(@.adapterType == 'dashscope-native-rerank')].displayName")
            .isEqualTo("DashScope 原生重排")
